      ...
      {adding ? (
        <motion.form layout onSubmit={handleSubmit}>
          ...
          <div className='mt-1.5 flex items-center gap-2 justify-end'>
            <span className="py-1 text-xs font-medium rounded-md shadow-xs bg-pink-400/10 inset-ring inset-ring-pink-400/40 shadow-red-300/50">
              <button
                onClick={() => setAdding(false)}
                className='flex items-center gap-1 px-3 text-xs transition-colors text-neutral-200 hover:text-red-500'>
                Cancel
                <RxCross2 />
              </button>
            </span>
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className='p-3 border rounded cursor-grab border-neutral-600 bg-neutral-800 active:cursor-grabbing hover:bg-neutral-700/80'>
...
const DeleteBox = ({ title, headingColor, setCards }) => {
  const [active, setActive] = useState(false);
  ...
  return (
    <div className='w-60 shrink-0 flex-col ml-5 mb-3 justify-center'>
      <h3 className={`font-medium ${headingColor}`}>{title}</h3>
...
const DEFAULT_CARDS = [
  ...
  { title: 'Purge old microservices', id: '5', column: 'backlog' },

  // TODO
  { title: 'Research DB options for new microservices', id: '6', column: 'todo' },
  { title: 'Postmortem for outage', id: '7', column: 'todo' },
  { title: 'Sync with product on Q3 roadmap', id: '8', column: 'todo' },
  { title: 'Reconciliate with old microservices ', id: '9', column: 'todo' },
